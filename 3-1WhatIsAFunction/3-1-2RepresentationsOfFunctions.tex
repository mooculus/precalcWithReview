...
\begin{image}
\includegraphics[width=\textwidth]{WiaF-arrowDiagram.jpg}
\end{image}
...
\end{example}
 ...
 When we give functions as a formula, we also need to say which input values are allowed, that is, what the domain of the function is.  In this case, the domain is $50 \leq x \leq 85$.  If the domain is not given, the domain is assumed to be all the values for which the formula used to defined the function makes sense, that is, the implied domain.  
...
\begin{example}
Let $f(x)=5x-2$.  Find $f(1)$. Then, find $f(m)$, $f(k + 1)$, $f(2h)$, and $f(\bigstar)$.

\begin{explanation}
Recall that the notation $f(1)$ means to evaluate the function $f$ at the input $x=1$.  That is, $f(1)=5(1)-2=3$.

Similarly, $f(m)$ means to evaluate the function $f$ at the input $x = m$. That is, $f(m) = 5m- 2$. This expression contains no like terms, so it cannot be simplified further.

$f(k + 1) = 5(k + 1) - 2$. Distributing, we find that $5(k + 1) -2 = 5k + 5 - 2 = 5k + 3$, so $f(k + 1) = 5k +3$. 

$f(2h) = 5(2h) - 2$, and multiplying $5(2h)$ yields $10h$, so $f(2h) = 10h - 2$. 

Although $f(\bigstar)$ involves an unusual symbol, we can treat the symbol like a number. Thus, $f(\bigstar) = 5\bigstar - 2$.
...
\item arrow diagrams
\item graphs
\item formula with domain and range
\end{itemize}
