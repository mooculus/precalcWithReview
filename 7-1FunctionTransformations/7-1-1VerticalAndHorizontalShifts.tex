...
\end{image}
 ...
 \begin{tikzpicture}
     \begin{axis}
      % \addplot[samples=200,domain=-7:7, color=blue]{x^2};
       \addplot[samples=200,domain=-7:7, color=red]{x^2+4};
     \end{axis}
 \end{tikzpicture}
 \end{image}
  ...
  \item $g(x) = 2e^{-x} - 1$. \\[.5em]
    \begin{explanation}
      ...
      \begin{image}
        \begin{tikzpicture}
          \begin{axis}
            \addplot[samples=200,domain=-3:7, color=blue]{2*exp(-x)};
            \addplot[samples=200,domain=-3:7, color=red]{2*exp(-x)-1};
  ...
  \item $g(x) = \sin(x) + 2$. \\[.5em]
    \begin{explanation}
      ...
      \begin{image}
        \begin{tikzpicture}
          \begin{axis}
            \addplot[samples=200,domain=-7:7, color=blue]{sin(deg(x))};
            \addplot[samples=200,domain=-7:7, color=red]{sin(deg(x))+2};
  ...
  \item $g(x) = 2x-3$ \\[.5em]
    \begin{explanation}
      Granted, graphing a linear function poses little to no challenge, but understanding how things work in this setting might offer us some general insight on $y$-intercepts. If $f(x) = 2x$, then $g(x) = f(x)-3$. Graphing $g(x) = 2x$ is easier than easy: just a line with slope $2$ passing through the origin. And the shift down by $3$ units comes last, as you would expect:
        ...
        \begin{tikzpicture}
          \begin{axis}
            \addplot[domain=-7:7, color=blue]{2*x};
            \addplot[,domain=-7:7, color=red]{2*x-3};
  ...
  \item $g(x) = 4e^{x+3}$. \\[.5em]
    \begin{explanation}
      ...
      \begin{image}
        \begin{tikzpicture}
          \begin{axis}
           \addplot[samples=200,domain=-7:3, color=blue]{4*exp(x)};
           \addplot[samples=200,domain=-7:2, color=red]{4*exp(x+3)};
    ...
    \begin{explanation}
      Consider this time $f(x) = \sin x$. Then we have that $g(x) = f(x-(\pi/2))$, which means that to graph $y=g(x)$, we may take the graph of $y=\sin x$ and shift it to the right by $\pi/2$ units. We obtain:
        ...
        \begin{tikzpicture}
          \begin{axis}
            \addplot[samples=200,domain=-7:7, color=blue]{sin(deg(x))};
            \addplot[samples=200,domain=-7:7, color=red]{sin(deg(x - pi/2))};
  ...
  \item $g(x) = (x-2)^2 - 5(x-2)+6$. \\[.5em]
    \begin{explanation}
      ...
      \begin{image}
        \begin{tikzpicture}
          \begin{axis}
            \addplot[samples=200,domain=-7:7, color=blue]{x^2-5*x+6};
            \addplot[samples=200,domain=-7:7, color=red]{(x-2)^2-5*(x-2)+6};
