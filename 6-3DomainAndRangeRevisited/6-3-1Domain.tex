		...
		\begin{tikzpicture}
  			\begin{axis}[numberline,
        			...
        			\addplot[mark=none] coordinates {(0,0)} node[below] {$\dfrac{4}{3}$};
                                \addplot[mark=none] coordinates {(0,0)} node[above] {\phantom{M}};
  			\end{axis}
		\end{tikzpicture}
	\end{image}
...
\section{Spotting Values not in the Domain}
	...
	\begin{callout}[Strategies for Finding the Domain]
		\begin{enumerate}
	...
	\begin{example}
		...
		\begin{explanation}
			...
			The points satisfying both inequalities are given in the graph found by overlaying both graphs above
