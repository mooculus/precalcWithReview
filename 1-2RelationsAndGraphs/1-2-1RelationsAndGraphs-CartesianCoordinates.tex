...
\begin{definition}
The \dfn{Cartesian coordinate system} is a coordinate system that specifies each point uniquely in a plane by a pair of numerical coordinates, which are the signed (positive/negative) distances to the point from two fixed perpendicular directed lines, measured in the same unit of length. Those two reference lines are called the \dfn{horizontal axis} and \dfn{vertical axis}, and the point where they meet is the \dfn{origin}. The horizontal and vertical axes are often called the \dfn{$x$-axis} and \dfn{$y$-axis}.  \\
