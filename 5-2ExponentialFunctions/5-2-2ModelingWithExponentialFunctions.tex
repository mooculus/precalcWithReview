...
A related situation arises when an object's temperature changes in response to its surroundings.  For instance, if we have a cup of coffee at an initial temperature of $186^\circ$ Fahrenheit and the cup is placed in a room where the surrounding temperature is $71^\circ$, our intuition and experience tell us that over time the coffee will cool and eventually tend to the $71^\circ$ temperature of the surroundings.  From an experiment with an actual temperature probe, we have the data in the table below.

\begin{center}
Data for cooling coffee\\
(measured in degrees Fahrenheit at the time t in minutes)\\
$$
\begin{array}{cc}
t&F(t)\\
\hline
...
\end{equation*}
In our work with transformations involving horizontal scaling, we saw that the graph of $y = h(-t)$ is the reflection of the graph of $y = h(t)$ across the $y$-axis.  Therefore, we can say that the graphs of $p(t) = 2^t$ and $q(t) = \left(\frac{1}{2}\right)^t = 2^{-t}$ are reflections of one another across the $y$-axis since $p(-t) = 2^{-t} = q(t)$.  We see this fact verified in the graph below.
