...
For the decreasing function \(q(t) = (\frac{1}{2})^t\), we see that the output \(q(t)\) is always positive but getting closer and closer to \(0\).  Indeed, becasue we can make \(2^t\) as large as we like, it follows that we can make its reciprocal \(\frac{1}{2^t} = (\frac{1}{2})^t\) as small as we'd like.  We thus say that as \(t\) increases, \(q(t)\) \dfn{approaches \(0\)}. \index{approaching \(0\)}
