...
\section{The Range of a Function}
	...
	\begin{example}
		...
		\begin{explanation}
			...
			To make that a bit more formal, we'll take an arbitrary real number and show that it's actually an output of $f$.
			...
			function, so $b$ is in the range.This means the range of $f$ is $(-\infty, \infty)$.
...
\section{Spotting Values not in the Range}
	...
	\begin{example}
		...
		\begin{explanation}
			...
			\begin{enumerate}
				\item We know that the range of $\sqrt{x}$ is $[0, \infty)$ so for any $x$ in the domain of $f$,
					\begin{align*}
						\sqrt{x} &\geq 0\\
						-4 \sqrt{x} &\leq 0 \\
						2 - 4\sqrt{x} &\leq 2 \\
						f(x) &\leq 2
					\end{align*}
					The outputs of $f$ are never larger than $2$, so the only numbers in the range are less than or equal to $2$. That is, the range must be inside the interval $(-\infty, 2]$.
				...
				\item The range of $e^x$ is $(0, \infty)$.
					That means for any value of $x$,
					\begin{align*}
						e^x &> 0\\
				...
				\item The range of $\sin$ is $[-1, 1]$. That means for any $x$
					\begin{align*}
						-1 &\leq \sin(x) \leq 1\\
						-5 &\leq 5\sin(x) \leq 5\\
					...
					\end{align*}
					That means the outputs of $h$ are in the interval $[-2, 8]$. To verify that the range is exactly $[-2, 8]$, suppose $b$ is a number with $-2 \leq b \leq 8$. Then:
					\begin{align*}
						h(a) &= b\\
						3 + 5\sin(a) &= b\\
						5\sin(a)& = b-3\\
