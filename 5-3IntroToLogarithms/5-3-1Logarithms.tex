...
%While for sine and cosine the familiar points come from specially placed points on the unit circle, 
...
\begin{definition}
Given a positive real number \(y\), the \dfn{natural logarithm of \(y\)} is the power to which we raise \(e\) to get \(y\).  We use the notation ``\(\ln(y)\)'' to denote the natural logarithm of \(y\). The domain of $\ln$ is all positive numbers, and the range of $\ln$ is all real numbers. 
\end{definition}
