...
\begin{itemize}
\item The domain of arcsecant is $\answer{(-\infty,-1]} \cup \answer{[1,\infty)}$.
%
\item The range of arcsecant is $\answer{[0,\frac{\pi}{2})} \cup \answer{(\frac{\pi}{2},\pi]}$.
\end{itemize}
%
\item Determine the following values exactly: 
\begin{itemize}
...
For $x$ in the domain of arcsecant, $\arcsec(x) = \arccos\!\Big(\frac{1}{x}\Big)$.
%
\item {\em Do we have a graph to include?}
%Below we have a plot of the restricted tangent function (in light blue) and its corresponding inverse, the arctangent function (in dark blue).%
\end{itemize}
%\includegraphics[width=1\linewidth]{inverse-trig-arctan-graph.pdf}
\end{callout}
...
Do we have a graph of arcsecant to include?
  \end{itemize}
\end{summary}
