...
\begin{document}
\begin{exercise}
Consider a rectangle whose lower corners are on the graph of $y = x^2$ under the line $y = 12$.
\begin{image}
\begin{tikzpicture}
	\begin{axis}[
          ...
          \addplot [very thick,penColor2,domain=(0:3)] {x*x};
	\addplot [very thick,penColor2,domain=(-3:0)] {x*x};
          ...
          \node at (axis cs:1.5,7.2) [anchor=west,penColor] {$(x,12)$};
        \end{axis}
\end{tikzpicture}
\end{image}

Say the rectangle has an area of 32 units$^2$.

The rectangle is $\answer{4}$ units wide and $\answer{8}$ units high.
